...
func (vd *VideoDecoder) Setup() (err error) {
	ff := &vd.ff.ff
	if len(vd.Extradata) > 0 {
		ff.codecCtx.extradata = (*C.uint8_t)(unsafe.Pointer(&vd.Extradata[0]))
		ff.codecCtx.extradata_size = C.int(len(vd.Extradata))
	}
...
func (vf *VideoFrame) Free() {
	vf.Image = image.YCbCr{}
	C.av_frame_free(&vf.frame)
}
...
func (vd *VideoDecoder) Decode(pkt []byte) (img *VideoFrame, err error) {
	ff := &vd.ff.ff
...
func (vd *VideoDecoder) DecodeSingle(pkt []byte) (img *VideoFrame, err error) {
	ff := &vd.ff.ff
	cgotimg := C.int(0)
	frame := C.av_frame_alloc()
	cerr := C.wrap_avcodec_decode_video2(ff.codecCtx, frame, unsafe.Pointer(&pkt[0]), C.int(len(pkt)), &cgotimg)
	...
	// https://stackoverflow.com/questions/25431413
	if cgotimg == C.int(0) {
