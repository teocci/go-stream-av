...
func (ae *AudioEncoder) SetSampleFormat(fmt av.SampleFormat) (err error) {
	ae.SampleFormat = fmt
	return
}

func (ae *AudioEncoder) SetSampleRate(rate int) (err error) {
	ae.SampleRate = rate
	return
}

func (ae *AudioEncoder) SetChannelLayout(ch av.ChannelLayout) (err error) {
	ae.ChannelLayout = ch
	return
}

func (ae *AudioEncoder) SetBitrate(bitrate int) (err error) {
	ae.Bitrate = bitrate
	return
}

func (ae *AudioEncoder) SetOption(key string, val interface{}) (err error) {
	ff := &ae.ff.ff
...
func (ae *AudioEncoder) GetOption(key string, val interface{}) (err error) {
	ff := &ae.ff.ff
...
func (ae *AudioEncoder) Setup() (err error) {
	ff := &ae.ff.ff

	ff.frame = C.av_frame_alloc()

	if ae.SampleFormat == av.SampleFormat(0) {
		ae.SampleFormat = sampleFormatFF2AV(*ff.codec.sample_fmts)
	}
	...
	if ae.ChannelLayout == av.ChannelLayout(0) {
		ae.ChannelLayout = av.CH_STEREO
	}

	ff.codecCtx.sample_fmt = sampleFormatAV2FF(ae.SampleFormat)
	ff.codecCtx.sample_rate = C.int(ae.SampleRate)
	ff.codecCtx.bit_rate = C.int64_t(ae.Bitrate)
	ff.codecCtx.channel_layout = channelLayoutAV2FF(ae.ChannelLayout)
	...
	ae.SampleFormat = sampleFormatFF2AV(ff.codecCtx.sample_fmt)
	ae.FrameSampleCount = int(ff.codecCtx.frame_size)
	...
	case C.AV_CODEC_ID_AAC:
		if ae.codecData, err = aacparser.NewCodecDataFromMPEG4AudioConfigBytes(extradata); err != nil {
			return
		}

	default:
		ae.codecData = audioCodecData{
			channelLayout: ae.ChannelLayout,
			sampleFormat:  ae.SampleFormat,
...
func (ae *AudioEncoder) prepare() (err error) {
	ff := &ae.ff.ff

	if ff.frame == nil {
		if err = ae.Setup(); err != nil {
			return
		}
	}
...
func (ae *AudioEncoder) CodecData() (codec av.AudioCodecData, err error) {
	if err = ae.prepare(); err != nil {
		return
	}
	codec = ae.codecData
	return
...
func (ae *AudioEncoder) encodeOne(frame av.AudioFrame) (gotpkt bool, pkt []byte, err error) {
	if err = ae.prepare(); err != nil {
		return
	}
...
func (ae *AudioEncoder) resample(in av.AudioFrame) (out av.AudioFrame, err error) {
	if ae.resampler == nil {
		ae.resampler = &Resampler{
			OutSampleFormat:  ae.SampleFormat,
			OutSampleRate:    ae.SampleRate,
			OutChannelLayout: ae.ChannelLayout,
		}
	}
	if out, err = ae.resampler.Resample(in); err != nil {
		return
	}
	return
}

func (ae *AudioEncoder) Encode(frame av.AudioFrame) (pkts [][]byte, err error) {
	var gotpkt bool
	var pkt []byte

	if frame.SampleFormat != ae.SampleFormat || frame.ChannelLayout != ae.ChannelLayout || frame.SampleRate != ae.SampleRate {
		if frame, err = ae.resample(frame); err != nil {
			return
		}
	}

	if ae.FrameSampleCount != 0 {
		if ae.framebuf.SampleCount == 0 {
			ae.framebuf = frame
		} else {
			ae.framebuf = ae.framebuf.Concat(frame)
		}
		for ae.framebuf.SampleCount >= ae.FrameSampleCount {
			frame := ae.framebuf.Slice(0, ae.FrameSampleCount)
			if gotpkt, pkt, err = ae.encodeOne(frame); err != nil {
				return
			}
			if gotpkt {
				pkts = append(pkts, pkt)
			}
			ae.framebuf = ae.framebuf.Slice(ae.FrameSampleCount, ae.framebuf.SampleCount)
		}
	} else {
		if gotpkt, pkt, err = ae.encodeOne(frame); err != nil {
			return
