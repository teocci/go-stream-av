...
func (p PAT) Len() (n int) {
	return len(p.Entries) * 4
}

func (p PAT) Marshal(b []byte) (n int) {
...
func (p *PAT) Unmarshal(b []byte) (n int, err error) {
	for n < len(b) {
		if n+4 <= len(b) {
			var entry PATEntry
...
func (p PMT) Len() (n int) {
	...
	for _, desc := range p.ProgramDescriptors {
		n += 2 + len(desc.Data)
	}

	for _, info := range p.ElementaryStreamInfos {
		// streamType
		n += 1
...
func (p PMT) fillDescs(b []byte, descs []Descriptor) (n int) {
...
func (p PMT) Marshal(b []byte) (n int) {
	// 111(3)
	// PCRPID(13)
	pio.PutU16BE(b[n:], p.PCRPID|7<<13)
	n += 2

	hold := n
	n += 2
	pos := n
	n += p.fillDescs(b[n:], p.ProgramDescriptors)
	desclen := n - pos
	...
	for _, info := range p.ElementaryStreamInfos {
		...
		n += 2
		pos := n
		n += p.fillDescs(b[n:], info.Descriptors)
		desclen := n - pos
...
func (p PMT) parseDescs(b []byte) (descs []Descriptor, err error) {
	n := 0
	for n < len(b) {
		if n+2 <= len(b) {
			desc := Descriptor{}
...
func (p *PMT) Unmarshal(b []byte) (n int, err error) {
	if len(b) < n+4 {
		err = ErrParsePMT
		return
	}
	...
	// 111(3)
	// PCRPID(13)
	p.PCRPID = pio.U16BE(b[0:2]) & 0x1fff
	n += 2
	...
	if desclen > 0 {
		...
		if p.ProgramDescriptors, err = p.parseDescs(b[n : n+desclen]); err != nil {
			return
		}
		n += desclen
	}
	...
	for n < len(b) {
		...
		if desclen > 0 {
			...
			if info.Descriptors, err = p.parseDescs(b[n : n+desclen]); err != nil {
				return
			}
			n += desclen
		}

		p.ElementaryStreamInfos = append(p.ElementaryStreamInfos, info)
	}
