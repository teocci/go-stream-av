...
import (
	...
	"github.com/teocci/go-stream-av/codec/h265parser"
...
func (client *RTSPClient) FirstTrack() av.CodecData {
	if client.CountTracks() > 0 {
		return client.CodecData[0]
	}
...
func (client *RTSPClient) CountTracks() int {
	return len(client.CodecData)
}

func (client *RTSPClient) IsAudioOnly() bool {
	return client.CountTracks() == 1 && client.FirstTrack().Type().IsAudio()
}
