...
func (cd CodecData) SPS() []byte {
	return cd.RecordInfo.SPS[0]
}

func (cd CodecData) PPS() []byte {
	return cd.RecordInfo.PPS[0]
}

func (cd CodecData) Width() int {
	return int(cd.SPSInfo.Width)
}

func (cd CodecData) Height() int {
	return int(cd.SPSInfo.Height)
}

func (cd CodecData) EmptySPS() bool {
	return len(cd.RecordInfo.SPS[0]) == 0
}

func (cd CodecData) EmptyPPS() bool {
	return len(cd.RecordInfo.PPS[0]) == 0
}
...
func NewCodecDataFromAVCDecoderConfRecord(record []byte) (codecData CodecData, err error) {
	codecData.Record = record
	if _, err = (&codecData.RecordInfo).Unmarshal(record); err != nil {
		return
	}
	if len(codecData.RecordInfo.SPS) == 0 {
		err = fmt.Errorf("h264parser: no SPS found in AVCDecoderConfRecord")
		return
	}
	if len(codecData.RecordInfo.PPS) == 0 {
		err = fmt.Errorf("h264parser: no PPS found in AVCDecoderConfRecord")
		return
	}
	if codecData.SPSInfo, err = ParseSPS(codecData.RecordInfo.SPS[0]); err != nil {
