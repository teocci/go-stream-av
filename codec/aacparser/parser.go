...
import (
	...
	"github.com/teocci/go-stream-av/utils/bits"
)
...
func (mac MPEG4AudioConfig) IsValid() bool {
	return mac.ObjectType > 0
}

func (mac *MPEG4AudioConfig) Complete() {
	if int(mac.SampleRateIndex) < len(sampleRateTable) {
		mac.SampleRate = sampleRateTable[mac.SampleRateIndex]
	}
	if int(mac.ChannelConfig) < len(chanConfigTable) {
		mac.ChannelLayout = chanConfigTable[mac.ChannelConfig]
	}
	return
}
...
func (cd CodecData) MPEG4AudioConfigBytes() []byte {
...
func (cd CodecData) PacketDuration(data []byte) (dur time.Duration, err error) {
	dur = time.Duration(1024) * time.Second / time.Duration(cd.Config.SampleRate)
	return
}
